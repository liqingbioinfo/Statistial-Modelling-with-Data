...
We can calculate the least squares point estimates $\hat{\beta}_0, \hat{\beta}_1,\hat{\beta_2},..., \hat{\beta_p}$ of the parameters $\beta_0,\beta_1,...,\beta_p$ in the model by using the following matrix algebra formula.
...
*y*=$\begin{bmatrix}y_1 \\ y_2 \\ y_3 \\ . \\ . \\ . \\ y_p \end{bmatrix}$ and *X*= $\begin{pmatrix}1 & x_{11} & x_{12} & . & . & . & x_{1p}\\1 & x_{21} & x_{22} & . & . & . & x_{2p}\\. & . & . & . & . & . & .\\. & . & . & . & . & . & .\\. & . & . & . & . & . & .\\1 & x_{n1} & x_{n2} & . & . & . & x_{np} \end{pmatrix}$

The **least squares estimates (LSE)** $\hat{\beta}_0, \hat{\beta}_1,\hat{\beta_2},..., \hat{\beta_p}$ are obtained by **minimizing the sum of the squared residuals:**
...
![Regresson line become a plane for 2 independent variables](/Users/pgalpern/OneDrive%20-%20University%20of%20Calgary/Teaching/Data603/data/datasets_pictures/multi1.png)

*In a three-dimensional setting, with 2 independent variables and one dependent varibale, the least square regression line becomes a plane. The plane is chosen to minimize the sum of the squared vertical distances between each observation (shown in red) and the plane.*

The values $\hat{\beta}_0,\hat{\beta}_1,\hat{\beta}_2,...,\hat{\beta}_p$ are called **the multiple least squares regression coefficient estimates.**
...
$\hat{\beta_1}=0.046$ means that for a given amount of radio and newspaper advertising, spending additional \$1,000 on TV advertising leads to an *increase* in sales by approximately 46 units.

$\hat{\beta_2}=0.189$ means that for a given amount of TV and newspaper advertising, spending additional \$1,000 on radio advertising leads to an *increase* in sales by approximately 189 units.

$\hat{\beta_3}=-0.001$ means that for a given amount of TV and radio advertising, spending additional \$1,000 on newspapers advertising leads to a *decrease* in sales by approximately 1 unit !!!!
...
#option 1
mulreg<-lm(listprice~listprice+livingarea+floors+bedrooms+baths,data=condominium)
summary(mulreg)
confint(mulreg) # a 95% confidence interval for coefficients


#option 2
condominium1=condominium[,-1]
reg<-lm(listprice~., data=condominium1)
summary(reg)
confint(reg) # a 95% confidence interval for coefficients
...
![The Analysis of Variance for Multiple Linear Regression](/Users/pgalpern/OneDrive%20-%20University%20of%20Calgary/Teaching/Data603/data/datasets_pictures/anovaMR.png)
...
![Tha Anova table for Advertising data example](/Users/pgalpern/OneDrive%20-%20University%20of%20Calgary/Teaching/Data603/data/datasets_pictures/anovaadvertisingexample.jpg)

From the Advertising example, the output shows that Fcal=570.3 with df= 3,196 (p-value\< 2.2e-16 \< $\alpha=0.05$ ),indicating that we should clearly reject the null hypothesis. It provides compelling evidence against the null hypothesis H~0~. In other word, the large F-test suggests that at least one of the advertising media must be related to sales. Based on the p-value, we also have extremely strong evidence that at least one of the media is associated with increased sales.
...
full<-lm(listprice~livingarea+floors+baths+bedrooms, data=condo) # full model
summary(full)


reduced1<-lm(listprice~livingarea+floors+baths,data=condo) #reduced model (dropping a bedrooms availabe)
summary(reduced1)

anova(reduced1,full)

reduced2<-lm(listprice~livingarea+baths,data=condo) #reduced model (dropping a bedrooms and a floors availabe)
summary(reduced2)
...
From the condominium problem, use the method of Model Fit to calculate $R^2_{adj}$ and RMSE for all possible models. Which model or set of models would you suggest for predictive purpose?

```{r,include=F}
condo=read.csv("condominium.csv",header = TRUE)
full<-lm(listprice~livingarea+floors+bedrooms+baths, data=condo) # full model
print("Full")
summary(full)$adj.r.squared
summary(full)$sigma
print("Reduced1")
reduced<-lm(listprice~livingarea+floors+baths,data=condo) #reduced model (dropping a bedrooms availabe)
summary(reduced)$adj.r.squared
